...
/// Claim reward - user burns access token and receives rewards.
/// Maps from Rewards.sol: claimReward -> _claimReward -> _distributeReward
///
/// Flow:
/// 1. Verify user holds the access token (Task #6 will add burn CPI)
...
fn distribute_rewards<'info>(
    ...
    for reward_idx in 0..state.rewards.len() {
        let reward = &state.rewards[reward_idx];

        match reward.reward_type {
            RewardType::Sol => {
                // Transfer SOL (lamports) from treasury PDA to recipient
                let amount = reward.amount;
                let treasury_ai = treasury_state.to_account_info();
            ...
            RewardType::SplToken => {
                ...
                require!(
                    remaining_idx + 3 <= remaining_accounts.len(),
                    SummonRewardsError::InvalidInput
                );

                let treasury_token_ai = &remaining_accounts[remaining_idx];
                let user_token_ai = &remaining_accounts[remaining_idx + 1];
                let reservation_ai = &remaining_accounts[remaining_idx + 2];
                remaining_idx += 3;
            ...
            RewardType::Nft => {
                ...
                for j in 0..nft_count {
                    ...
                    let treasury_nft_ai = &remaining_accounts[remaining_idx];
                    let user_nft_ai = &remaining_accounts[remaining_idx + 1];
                    let nft_reservation_ai = &remaining_accounts[remaining_idx + 2];
                    remaining_idx += 3;
                    ...
                    nft_reservation.exit(program_id)?;
                }

                // Increment the NFT current index for this reward
                // We need mutable access to update nft_current_index
                state.rewards[reward_idx].nft_current_index = (current_index + nft_count) as u64;
            }
        }
    }
